        ...
        float distance_from_0(float x1, float x2){
        ...
        void main(void) {
           ...
           vec4 textureColor = texture2D(uSampler, vec2(uv.s, uv.t));
            ...
            float z=position.z;
            float y=position.y;
            float col_r=textureColor.x;
            float col_g=textureColor.z;
            float col_b=textureColor.y;

            if((col_g < col_b && col_g < col_r) || (sqrt((1.-col_b)*(1.-col_b)+(1.-col_r)*(1.-col_r)+(1.-col_g)*(1.-col_g))<0.1)){
              position.x=1.1*x;
              position.y=1.1*y;
              position.z=1.1*z;
            } else {
              position+=position*0.02*sin(-20.*PI*uv.x-time*6.*PI);
              position+=position*0.02*sin(-20.*PI*uv.y-time*6.*PI);
            }
