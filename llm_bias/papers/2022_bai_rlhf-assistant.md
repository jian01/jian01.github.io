...
**Etapa 4 — RL con PPO:** El asistente se fine-tunea con Proximal Policy Optimization (PPO), un algoritmo de reinforcement learning. En cada paso, el modelo genera respuestas, el reward model las califica, y el gradiente de RL actualiza los parámetros del asistente para maximizar la recompensa. Una restricción KL previene que el modelo se desvíe demasiado de la **política SFT** — que en este paper no es un modelo entrenado sobre demostraciones humanas (como en InstructGPT), sino el modelo obtenido por **context distillation**: se parte del LM base y se hace fine-tuning para imitar las respuestas que produciría ese mismo modelo cuando el contexto incluye un prefijo HHH ("I am a helpful, harmless, and honest assistant..."). Este modelo destilado es más barato de obtener que recopilar miles de demostraciones humanas y sirve como punto de partida e inicialización de referencia para el entrenamiento PPO.
